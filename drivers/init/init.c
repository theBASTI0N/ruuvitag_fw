 ...
 * Initialize Switch
 *
 * This function initializes GPIO for for a fitted reed switch, setting pull-up and sensing hi-to-low transition.
 *
 */
init_err_code_t init_sw(void)
{   
    nrf_gpio_cfg_sense_input(SW, BUTTON_PULL, NRF_GPIO_PIN_SENSE_LOW);
    return INIT_SUCCESS; // Cannot fail under any reasonable circumstance
}
